-- Supabase Schema setup for Art of Living Centre

-- Enable pgcron for 14-day archive deletion if not already enabled
...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  phone_number TEXT,
  photo_url TEXT,
  responsibilities TEXT[] DEFAULT '{"Volunteer"}'::TEXT[],
...
DECLARE
  ...
  v_resps TEXT[];
BEGIN
  -- Safely extract name, defaulting to email prefix if not provided
  v_name := COALESCE(new.raw_user_meta_data->>'name', split_part(new.email, '@', 1));
  
  -- Safely extract responsibilities. 
  -- When creating the very first Administrator manually via Supabase dashboard, metadata will be empty.
  ...
  ELSE
    -- Safe fallback so the trigger doesn't crash on manual creation
...
-- Note: the trigger is created but we might manually create profiles from the edge function
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
CREATE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE PROCEDURE public.handle_new_user();

-- Helper function to check responsibilities
...
-- Ensure at least one manager on creation as well
CREATE OR REPLACE FUNCTION public.check_course_managers_count_deferred() 
RETURNS TRIGGER AS $$
BEGIN
  -- For delete operations on managers
  IF TG_OP = 'DELETE' THEN
    ...
    END IF;
  -- For insert operations on courses
...
-- RPC for atomic course creation
...
DECLARE
  ...
  v_caller_resps TEXT[];
BEGIN
  -- Verify caller is authorized and active
  ...
  -- Verify manager is an active Teacher
  ...
  -- Automatically add creator as a manager if they are a Teacher and not already the initial manager
...
BEGIN
  ...
  -- Verify manager is an active Teacher or Volunteer
  SELECT responsibilities INTO v_resps FROM public.profiles WHERE id = p_manager_id AND archived_at IS NULL;
  ...
  -- Automatically add creator as a manager if they are Staff and not already the initial manager
...
-- Public can read active (non-archived) content
...
-- Storage Buckets (Optional, if using Supabase Storage for Gallery/Photos)
-- insert into storage.buckets (id, name, public) values ('gallery', 'gallery', true);
-- insert into storage.buckets (id, name, public) values ('profiles', 'profiles', true);
-- CREATE POLICY "Public Access" ON storage.objects FOR SELECT USING ( bucket_id = 'gallery' );
-- CREATE POLICY "Staff Uploads" ON storage.objects FOR INSERT WITH CHECK ( bucket_id = 'gallery' AND auth.role() = 'authenticated' );

-- 14-day Auto-Delete Function (can be scheduled via pg_cron or called from Edge Function)
...
BEGIN
  ...
  -- Also delete archived profiles (this would cascade delete if relations are set up, but auth.users deletion needs admin API)
END;
$$ LANGUAGE plpgsql;

-- Administrator Protection Trigger
...
BEGIN
  ...
  -- Prevent archiving Administrator
  IF TG_OP = 'UPDATE' THEN
    ...
    -- Track new Administrator assignments
    ...
    -- Prevent removing Admin role unless by self or within 3 days
    IF 'Administrator' = ANY(OLD.responsibilities) AND NOT ('Administrator' = ANY(NEW.responsibilities)) THEN
      -- Allow if it's the user themselves (Return Administrative Access)
      IF auth.uid() = OLD.id THEN
        RETURN NEW;
      END IF;

      -- Otherwise, check if there is an assignment within 3 days granted by the caller
      IF NOT EXISTS (
...
-- Enable pg_cron extension if not already enabled (Requires Supabase Pro/Team, or explicit enable in Dashboard)
CREATE EXTENSION IF NOT EXISTS pg_cron;

-- Schedule the cleanup function to run every day at midnight
-- Safely unschedule if it exists to make it idempotent
DO $$
BEGIN
  PERFORM cron.unschedule('delete_expired_archive_items');
EXCEPTION
  WHEN OTHERS THEN
    -- Ignore errors if pg_cron is not properly configured yet or job doesn't exist
    NULL;
END $$;

SELECT cron.schedule(
  'delete_expired_archive_items', 
  '0 0 * * *', 
  $$SELECT public.delete_old_archived_items()$$
);
